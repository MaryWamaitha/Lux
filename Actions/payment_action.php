...
if (isset($_GET['delivery'])){
    ...
    $add_order = add_order_controller($custID,$invoice_no,$date,$status,$amount);
    ...
    if($add_order){
        ...
        foreach($cart as $x){
            $details=$x['details'];
            $size=$x['size'];
            $add_OrderDetails = add_order_details_controller($recent_order['currentOrder'],$x['p_id'],$x['qty'],$details,$size);
        }
...
CURLOPT_HTTPHEADER => array(
    "Authorization: Bearer ssk_live_497a3a223893acf3ff8ecfd4dce1158b2fc9b088",
       ...
       foreach($cart as $x){
        ...
        if($add_Payment){
            ...
            if($clearCart){
                ...
                header("Location: ../view/payment.php?error=2");

            }
        } else{
            header("Location: ../view/payment.php?error=3");
        }
    } else{
        header("Location: ../view/payment.php?error=0");
    }
